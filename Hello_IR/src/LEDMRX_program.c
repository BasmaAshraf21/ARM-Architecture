...
void HLEDMRX_Player(u8 *Copy_u8Data)
{

		for(u8 i = 0; i < 3; i++)
		{
			u8 x = 0;
			u8 Local_Data[8] ={Copy_u8Data[i*8],Copy_u8Data[i*8 + 1],Copy_u8Data[i*8 +2],Copy_u8Data[i*8 +3],Copy_u8Data[i*8 +4],Copy_u8Data[i*8 +5],Copy_u8Data[i*8 +6],Copy_u8Data[i*8 +7]};
			while (x < 50)
			{
				x++;
				for(u8 k = 0; k < 8; k++)
				{
					...
					SetRowValue(Local_Data[k]);
					MGPIO_voidSetPinValue(cols[k*2],cols[k*2 + 1],LOW);
					MSTK_voidSetBusyWait(2800); //2.5 msec
